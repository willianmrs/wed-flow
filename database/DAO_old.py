...
class DAO:
    ...
    def create_tables(self):
        fodase=type("fodase", (Base,), {'__tablename__' : 'fodase', 'id': Column(Integer, primary_key = True)})
    ...
    def insert(self):
        list_attributes = DAO.readxml.data_wed_attributes()
        ...
        list_conditions = DAO.readxml.data_wed_conditions()
        for condition in list_conditions:
            self.session.add(condition)
            self.session.commit()        

        list_transitions = DAO.readxml.data_wed_transitions()
        for transitions in list_transitions:
            self.session.add(transitions)
            self.session.commit()
                                
        list_flows = DAO.readxml.data_wed_flows()
        for flows in list_flows:
            self.session.add(flows)
            self.session.commit()        

        list_trigger = DAO.readxml.data_wed_trigger()
    ...
    def select_flow(self, wed_flow):
        ...
        return result        
#dao = DAO()
#b = DAO.readxml('../xml/B1.xml')
#dao.set_DAO.readxml(b)
#dao.drop_tables
#dao.insert()
