...
class Readxml:
    ...
    def data_wed_flows(self):
        ...
        for data_flows in d:
            ...
            result = Readxml.dao.select_condition(wed_condition)
    ...
    def data_wed_trigger(self):
        ...
        for data_flows in d:
            result_flow = Readxml.dao.select_flow(d['Flow']['@Name'])
            flow_id = result_flow[0]
            triggers = d['Flow']['Trigger']
            for tgg in triggers:
                result_cond_id = Readxml.dao.select_condition(tgg['@CondName'])
                cond_id = result_cond_id[0]
                result_trans_id = Readxml.dao.select_transition(tgg['@TransName'])
